...
-- =========================
-- TRG_CHECK_SERIAL_UNIQUE_ENTITY
-- Vérifie l'unicité du numéro de série au sein d'une même entité
-- =========================

CREATE OR REPLACE TRIGGER TRG_CHECK_SERIAL_COMPUTER
BEFORE INSERT OR UPDATE ON computers
FOR EACH ROW
DECLARE
    v_count NUMBER;
BEGIN
    IF :NEW.serial IS NOT NULL THEN
        ...
        WHERE serial = :NEW.serial
          AND entities_id = :NEW.entities_id
          AND id != NVL(:NEW.id, -1);

        IF v_count > 0 THEN
            RAISE_APPLICATION_ERROR(-20003,
                'Le numéro de série "' || :NEW.serial ||
                '" existe déjà dans l''entité ' || :NEW.entities_id);
        END IF;
    END IF;
END;
/
